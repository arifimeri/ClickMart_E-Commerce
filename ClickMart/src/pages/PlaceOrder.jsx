...
const PlaceOrder = () => {
  ...
  // This function will ONLY run if all required inputs are filled out correctly
  const handleSubmit = (event) => {
    event.preventDefault(); // Prevents the page from refreshing
    
    // Put your order submission logic here (e.g., API call to save order)
    
    navigate('/orders'); // Redirect to orders page
  };

  return (
    /* 1. Wrapped EVERYTHING in a single <form> with onSubmit */
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      
      {/* Left Side (Now a standard div again, inside the form) */}
      ...
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
                {/* We use type="button" on these divs if needed, but since they are divs they won't trigger submits */}
                <div onClick={()=>setMethod('stripe')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
                  <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
                  <img className="h-10 mx-1" src={assets.stripeSymbol} alt="StripeSymbol" />
                </div>
                <div onClick={()=>setMethod('raiffeisen')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
                  <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'raiffeisen' ? 'bg-green-400' : ''}`}></p>
                  <img className="h-10 mx-1" src={assets.raiffieisenBankLogo} alt="RaiffieisenBankLogo" />
                </div>
